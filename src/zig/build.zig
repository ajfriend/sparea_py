...
pub fn build(b: *std.Build) void {
    ...
    // Static lib: gets pulled into the consumer (the C++ binding's
    // .pyd / .so / .dylib) at link time. Avoids the Windows MSVC CRT
    // mismatch you get when mixing a Zig DLL with a separately-
    // compiled MSVC C++ extension (zig#19672, #18685, #11422), and
    // sidesteps the macOS dylib `__dso_handle` regression (zig#24370).
